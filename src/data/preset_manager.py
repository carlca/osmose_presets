...
class PresetDataManager:
        ...
        try:
            ...
            for i, item in enumerate(data):
                try:
                    ...
                    logger.warning(f"Failed to load preset at index {i}: {e}")
                    # Continue loading other presets
                    
            self._loaded = True
            logger.info(f"Loaded {len(self._presets)} presets from {self.file_path}")
    ...
    def get_preset_max_widths(self) -> List[int]:
        ...
        field_names = [f.name if f.name != "chars" else "character" 
    ...
    def __repr__(self) -> str:
        ...
        return (f"PresetDataManager(file={self.file_path.name}, "
                f"loaded={self._loaded}, count={len(self._presets)})")