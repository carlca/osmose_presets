...
def _get_context() -> AppContext:
    ...
    if _app_context is None:
        ...
        logger.info("Initialized compatibility app context")
    return _app_context
...
class PresetData:
    ...
    def evaluate_search(search_term: str, target: str) -> bool:
        ...
        and_results = (
            all(term.strip().lower() in target.lower() 
    ...
    def get_presets():
        ...
        return manager.get_filtered_presets(
            packs=packs,
            types=types,
            chars=chars,
            search_term=PresetData.search_term
        )
...
if not PresetData.cached_presets:
    try:
        ...
        if preset_file.exists():
            ...
            logger.info(f"Loaded {len(PresetData.cached_presets)} presets for compatibility")
    except Exception as e:
        logger.warning(f"Could not load presets on import: {e}")
