...
def main():
   ...
   parser.add_argument(
      ...
      nargs="?",
      default="standard",
      choices=["standard", "verbose", "coverage", "integration", "search", "filters", "quick", "full", "failed"],
      help="Test mode to run",
   )
   parser.add_argument("--html", action="store_true", help="Generate HTML coverage report")
   parser.add_argument("--file", type=str, help="Run tests for a specific file")
   parser.add_argument("--class", type=str, dest="test_class", help="Run tests for a specific test class")
   ...
   commands = {
      "standard": {"cmd": f"python3 -m pytest {test_dir}/test_preset_model.py -v", "desc": "Standard test run with moderate verbosity"},
      "verbose": {
      ...
      "integration": {"cmd": f"python3 -m pytest {test_dir}/test_preset_integration.py -vvs", "desc": "Integration tests with real data"},
      "search": {"cmd": f"python3 -m pytest {test_dir}/test_preset_model.py::TestPresetSearch -vvs", "desc": "Search functionality tests only"},
      "filters": {"cmd": f"python3 -m pytest {test_dir}/test_preset_model.py::TestPresetFilters -vvs", "desc": "Filter functionality tests only"},
      "quick": {"cmd": f"python3 -m pytest {test_dir}/test_preset_model.py -q", "desc": "Quick test run with minimal output"},
      "full": {"cmd": f"python3 -m pytest {test_dir}/ -vvs --tb=short", "desc": "Run all tests with full verbosity"},
      "failed": {"cmd": f"python3 -m pytest {test_dir}/ --lf -vv", "desc": "Run only previously failed tests"},
   }
